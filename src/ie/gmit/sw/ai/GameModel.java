...
public class GameModel {
	...
	private char[][] model;
	private enemy eh = new enemy();
	private playerHealth ph = new playerHealth();
	private NetworkLogicController NLC = new NetworkLogicController();
	...
	private void addGameCharacter(Collection<Task<Void>> tasks, char enemyID, char replace, int number){
		...
		while (counter < number){
			...
			if (model[row][col] == replace){
				 ...
				 */
				tasks.add(new CharacterTask(this, enemyID, row, col, ()-> System.out.print("")));
				counter++;
			}
		}
	}
	...
	public boolean isValidMove(int fromRow, int fromCol, int toRow, int toCol, char character){
		...
		}else if((toRow <= this.size() - 1 && toCol <= this.size() - 1 && this.get(toRow, toCol) == '1'))
		{
			this.set(fromRow, fromCol, '\u0020');
			this.set(toRow, toCol, character);
			//Enemy ID for Strength
			eh.setEnemyID(character);
			//Prediction by Neural Network
			int nnpredict = NLC.NNprediction(ph.getPlayerPower(), eh.getStrength());
			//If statement deciding what to do based on NN output
			if(nnpredict == 0)
			{
				//Powerup Player by 1 and print text to console
				ph.incrementPower();
				System.out.println("=========================================================");
				System.out.println("Ghost Defeated!");
				System.out.println("Strength Power Increased to: " + ph.getPlayerPower());
				System.out.println("Current Player Health: " + ph.getPlayerHealth());
				System.out.println("=========================================================");
				
			}
			else
			{
				//Player gets hit and it calls the method
				ph.playerHit();
				//Printing text to console
				System.out.println("=========================================================");
				System.out.println("Ghost Damaged Player!");
				System.out.println("Current Player Health: " + ph.getPlayerHealth());
				System.out.println("=========================================================");
			}
			//Incrementing the hitCounter
			hitCounter++;
			FuzzyController game = new FuzzyController();
			double output = game.getGameInfo(eh.getEnemyHealth(), ph.getPlayerHealth());
			if(output >= 100) 
			{
				//If player gets to 0hp - Game ends and closes
				System.out.println("=========================================================");
			...
			{
				//If the hitCounter is 6 or more the game ends in a win for the player
				System.out.println("=========================================================");
